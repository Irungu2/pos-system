...
class BulkRestockSuccess {
    ...
    // FIX: Get ID from Django template, not query params
    getRestockId() {
    ...
    updateUI() {
        ...
        if (restockIdEl) {
            restockIdEl.textContent = `#${this.restockData.id}`;
        }
        ...
        const totalQuantity = items.reduce((sum, item) => {
            return sum + (item.new_quantity || item.quantity_change || 0);
        }, 0);
        ...
        if (processedByEl && this.restockData.completed_by) {
            processedByEl.textContent =
                this.restockData.completed_by.username ||
                this.restockData.completed_by.email ||
                'System';
        }
        ...
        if (processedAtEl && this.restockData.updated_at) {
            processedAtEl.textContent =
                new Date(this.restockData.updated_at).toLocaleString();
        }
    }
